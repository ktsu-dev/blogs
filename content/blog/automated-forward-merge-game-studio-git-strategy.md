...
steps:
- checkout: self
  fetchDepth: 0  # Full history needed for merge

- task: PowerShell@2
  displayName: 'Forward-merge main to all branches'
  inputs:
    targetType: 'inline'
    script: |
      git config user.email "pipeline@studio.com"
      ...
      foreach ($branch in $branches) {
        ...
        if ($exitCode -ne 0) {
          ...
          # Post to Slack
          $payload = @{
            text = "Merge conflict: main -> $branch"
            blocks = @(
              @{ type = "section"; text = @{
                type = "mrkdwn"
                text = "*Conflict in branch:* ``$branch``\n*Files:*\n$($conflictFiles -join '\n')\n*Owners:* $($uniqueOwners -join ', ')"
              }}
            )
